...
*/
public class NationComp_Statistical_Data extends AsyncTask<NationComp_Statistical_Data_Structure, Void, String[]>
{
    //api 주소
    ...
    */
    ...
    //백그라운드 실행 (변수 초기화)
    @Override
    protected String[] doInBackground(NationComp_Statistical_Data_Structure... nation_comp_statistical_data_structures)
    {
        try
        {
            nation_comp_statistical_layout = nation_comp_statistical_data_structures[0].nation_comp_statistical_layout;

            title_num = nation_comp_statistical_data_structures[0].title_num;
            subtitle_num = nation_comp_statistical_data_structures[0].subtitle_num;
            ...
            local_num = nation_comp_statistical_data_structures[0].local_num;

            graph_background_view = nation_comp_statistical_data_structures[0].graph_background_view;
            graph_data_view = nation_comp_statistical_data_structures[0].graph_data_view;

            main_graph_bool = nation_comp_statistical_data_structures[0].main_graph_bool;
    ...
    protected void onPostExecute(String[] result)
    {
        ...
        //그래프 데이터 배열 할당
        graph_data_view.alloc_data_array(local_num.length);

        //그래프 데이터 수에 따른 배열 할당, 초기화
        graph_data_view.graph_data_array_init();

        for(int i=0;i<graph_data_view.graph_data_num;i++)
        {
            //StructureSpecific(구조화된 수치자료)에서 원하는 자료 추출 후 전송
            graph_data_view.data_value[i] =  Float.parseFloat(get_structure_specific_data(result[2], subtitle_num, local_num[i]));
        }

        //그래프 부제목, 지역 정보 입력
        graph_data_view.graph_element.set_local_data(title_string, subtitle_string, city_string_array(local_num));

        //그래프 배경 크기 설정
        graph_background_view.set_graph_list_size(nation_comp_statistical_layout);

        //그래프 위치 설정
        graph_background_view.set_list_graph_background_pos(-1);

        //그래프 데이터 크기정보 설정
        graph_data_view.set_graph_data_pos_and_size(graph_background_view.graph_element);

        //그래프 배경이 없을 경우
        if(main_graph_bool[0] == false)
        {
            //그래프 배경 그리기
            nation_comp_statistical_layout.addView(graph_background_view);
        }
        main_graph_bool[0] = true;

        //그래프 데이터가 있을 경우
        if(main_graph_bool[1] == true)
        {
            //그래프 데이터, 지우고 다시 그리기
            nation_comp_statistical_layout.removeView(nation_comp_statistical_layout.findViewById(VIEW_BASE_ID + 1));
        }
        nation_comp_statistical_layout.addView(graph_data_view);

        main_graph_bool[1] = true;
    }
