...
*/
public class Statistical_Data extends AsyncTask<Statistical_Data_Structure, Void, String[]>
{
    //api 주소
    ...
    */
    ...
    //백그라운드 실행 (변수 초기화)
    @Override
    protected String[] doInBackground(Statistical_Data_Structure... statistical_data_structures)
    {
        try
        {
            title_num = statistical_data_structures[0].title_num;
            subtitle_num = statistical_data_structures[0].subtitle_num;
            ...
            local_num = statistical_data_structures[0].local_num;

            main_map_layout = statistical_data_structures[0].main_map_layout;

            map_draw = statistical_data_structures[0].map_draw;

            graph_background_view = statistical_data_structures[0].graph_background_view;
            graph_data_view = statistical_data_structures[0].graph_data_view;

            VIEW_BASE_ID = statistical_data_structures[0].VIEW_BASE_ID;
            view_num = statistical_data_structures[0].view_num;

            main_graph_bool = new boolean[2];
            for(int i=0;i<2;i++)
            {
                main_graph_bool[i] = statistical_data_structures[0].main_graph_bool[i];
            }
    ...
    protected void onPostExecute(String[] result)
    {
        ...
        //그래프 배경 크기 설정
        graph_background_view.set_graph_base_size(main_map_layout);

        //그래프 위치 설정
        graph_background_view.set_main_graph_background_pos(main_map_layout, view_num);
        ...
        {
            //그래프 배경 그리기
            main_map_layout.addView(graph_background_view);
        }
        ...
        main_map_layout.addView(graph_data_view);

        //그래프 데이터 마커 그리기
        if(map_draw != null)
        {
            graph_data_view.map_marker(map_draw, view_num, local_num);
        }
